...
- **BELUM**: Users-web + Auth (keystone, buka jalan Orders) → Orders → Reviews/Quotations/CS-Sessions/Admin-Log → List Account* (Cek Status, boleh tetap GAS lama).
- **LANGKAH BERIKUTNYA (start sesi baru)**: setup **Supabase MCP resmi** (`@supabase/mcp-server-supabase`, butuh Personal Access Token dari Supabase + restart sesi) supaya migrasi skema/SQL bisa dijalankan langsung tanpa copy-paste manual. Registry MCP bawaan TIDAK punya connector Supabase. Alternatif: `psql` via connection string (berisi password DB — hanya jika owner nyaman). Setelah MCP aktif → kerjakan Auth.
